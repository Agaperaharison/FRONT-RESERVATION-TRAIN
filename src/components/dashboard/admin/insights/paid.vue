            ...
            <div class="progress">
                <svg>
                    ...
                    <circle cx="38" cy="38" r="36" class="animated-circle"
                        :style="{ strokeDashoffset: value > 0 ? (222 - ((this.totalPaid * 222) / this.totalAmount) - 3).toFixed(0) : '222px' }" />
                </svg>
                <div class="number">
                    <p>{{ value > 0 ? ((this.totalPaid * 100) / this.totalAmount).toFixed(2) : 0 }}%</p>
                </div>
            </div>
        </div>
    </div>
